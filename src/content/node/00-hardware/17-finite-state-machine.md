...
**有限状态机（Finite State Machine, FSM）** 是一个数学模型——系统在**有限个状态**之间转移，转移规则取决于当前状态和输入。

FSM 是 [[d-flipflop|D 触发器]] 的推广：从能存储 1 位（2 个状态）扩展到能存储 N 位（$2^N$ 个状态）。所有时序逻辑电路——[[counter|计数器]]、寄存器、CPU 控制器——本质上都是状态机。
