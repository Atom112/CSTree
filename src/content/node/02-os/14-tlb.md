...
TLB 对内存访问性能的影响有多大？来算一笔账：

假设：
- TLB 命中率：99%
- TLB 命中耗时：0.5 纳秒（~1 个 CPU 周期）
- TLB 未命中耗时：50 纳秒（需要查 4 级页表）
- 每次内存访问耗时（数据读取）：平均 100 纳秒

```
有 TLB 时有效访存耗时：
= TLB 命中率 × TLB 命中时间 + TLB 未命中率 × TLB未命中时间
= 99% × 0.5ns + 1% × 50ns
= 0.495ns + 0.5ns
≈ 1ns
...
结论：TLB 的 99% 命中率让虚拟内存的性能损失从 150× 降到了几乎可以忽略。
```

这就是为什么 TLB 是 CPU 内部最关键的加速结构之一——没有它，虚拟内存带来的性能开销是不可接受的。
